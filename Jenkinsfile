pipeline {
    ...
    environment {
          ...
          ECR_REPO_NAME = 'food-delivery-customer-service'  // Replace with your ECR repository name
//           IMAGE_TAG = "${env.BUILD_ID}"  // Or use 'latest' or any other tag
          IMAGE_TAG = "latest"
          WORKSPACE = "/var/lib/jenkins/workspace/food-delivery-customer-service"
    }
    stages {
        stage('Clone Repo to Build') {
            steps {
                git url: 'https://github.com/Topsan2002/food-delivery-customer-service.git',
                    branch:'master'
            }
        }
        ...
        stage('Deploy or Run Docker Container') {
            ...
            steps {
                script {
                    ...
                    def serviceName = "food-delivery-customer-service"
